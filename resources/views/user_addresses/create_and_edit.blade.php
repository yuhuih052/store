        ...
        <div class="col-lg-10 col-lg-offset-1">
            <div class="panel panel-default">
                ...
                <!-- 输出后端报错结束 -->
                    ...
                    @endif
                    <!--在页面中插入一个隐藏的 input，包含了提交表单所需要的 csrf token -->
                    {{ csrf_field() }}
                        <!-- inline-template 代表通过内联方式引入组件 -->
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-3">
                                    <select class="form-control" v-model="provinceId">
                                        <option value="">{{old('province', $address->province)?old('province', $address->province):'选择省'}}</option>
                                        <option v-for="(name, id) in provinces" :value="id">@{{ name }}</option>
                                    </select>
                                </div>
                                <div class="col-sm-3">
                                    <select class="form-control" v-model="cityId">
                                        <option value="">{{old('city', $address->city)?old('city', $address->city):'选择市'}}</option>
                                        <option v-for="(name, id) in cities" :value="id">@{{ name }}</option>
                                    </select>
                                </div>
                                <div class="col-sm-3">
                                    <select class="form-control" v-model="districtId">
                                        <option value="">{{old('district', $address->district)?old('district', $address->district):'选择区'}}</option>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <textarea rows="2" cols="15" class="form-control" name="address" style="resize:none;">{{ old('address', $address->address) }}</textarea>
