            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <span class="product-title">
                       <a target="_blank" href="{{ route('products.details', [$item->product_id]) }}">{{ $item->product->title }}</a>
                     </span>
                                                    ...
                                                    @if($index === 0)
                                                        ...
                                                        <td rowspan="{{ count($order->items) }}" class="text-center">
                                                            ...
                                                            @else
                                                                未支付<br>
                                                                请于 {{ $order->created_at->addSeconds(config('app.CloseOrderDelay'))->format('H:i') }} 前完成支付<br>
                                                                否则订单将自动关闭
                                                            @endif
                                                        </td>
                                                        <td rowspan="{{ count($order->items) }}" class="text-center"><a class="btn btn-primary btn-xs" href="{{ route('orders.details', ['order' => $order->id]) }}">查看订单</a></td>
                                                    @endif
                                                </tr>
                                            @endforeach
                                        </table>
